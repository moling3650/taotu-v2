...
export default {
  ...
  computed: {
    backgroundStyle () {
      return this.config ? { backgroundImage: `url('${this.config.background}')` } : {}
    },
    descriptionHtml () {
      return this.config ? (this.config.description || '').replace(/\n/g, '<br />') : ''
    }
  }
}
</script>
